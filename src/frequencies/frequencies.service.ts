...
export class FrequenciesService {
  ...
  async searchRoutes(searchRoutesDto: SearchRoutesDto): Promise<RouteSearchResponse[]> {
    ...
    console.log(`Buscando rutas: ${originCityId} -> ${destinationCityId} para ${searchDate.toISOString()}`);
    ...
    try {
      ...
      const routes: RouteSearchResponse[] = routeSheetDetails.map((routeSheet) => {
        const { frequency, bus, ticket } = routeSheet;

        // Calcular disponibilidad de asientos
        const seatsAvailability = this.calculateSeatsAvailability(routeSheet, ticket);
        ...
        const { duration, estimatedArrival } = this.calculateDurationAndArrival(
  ...
  private calculatePricing(routePrice: any) {
    ...
    const childDiscountPercent = parseFloat(routePrice.childDiscount.toString());
    const seniorDiscountPercent = parseFloat(routePrice.seniorDiscount.toString());
    const handicappedDiscountPercent = parseFloat(routePrice.handicappedDiscount.toString());
   ...
   */
  private calculateDurationAndArrival(departureTime: Date, originCity: string, destinationCity: string) {
  ...
  async searchRoutesMock(searchRoutesDto: SearchRoutesDto): Promise<RouteSearchResponse[]> {
    const { originCityId, destinationCityId, date } = searchRoutesDto;

    console.log(`Búsqueda MOCK: ${originCityId} -> ${destinationCityId} para ${date}`);
